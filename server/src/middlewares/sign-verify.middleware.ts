...
export class SignVerifyMiddleware {
    use(req: MRequest, res: Response, next: NextFunction) {
        ...
        if (!sign) throw new ExceptionResponse(HttpStatus.UNAUTHORIZED, 'Sign is not valid!');

        //generate serverSign depend on params and query to compare
        const serverSign = crypto.createHmac('sha256', process.env.SECRET_SIGN_KEY)
            .update(data)
            .digest('hex')

        if (sign != serverSign) throw new ExceptionResponse(HttpStatus.UNAUTHORIZED, 'Sign is not valid!');
        next()
    }
}